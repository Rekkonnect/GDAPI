...
namespace GDAPI.Functions.Crypto
{
    ...
    public static class GeometryDashCrypto
    {
        ...
        public static string GDGamesaveDecrypt(string data)
        {
            ...
            return Encoding.ASCII.GetString(gzipb64); // Change to string
        }
        ...
        {
            byte[] compressed = GZipCompression.Compress(Encoding.ASCII.GetBytes(data)); // Compress
        ...
        {
            byte[] compressed = GZipCompression.Compress(Encoding.ASCII.GetBytes(ls)); // Compress
